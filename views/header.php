...
<?if(Kohana::config('admin.header')):?>
	<div id="header_info" class="wrapper">
		<a href="<?=join_paths(Kohana::config("admin.base"), 'login/logout')?>" style="float:right; text-align:right;">Logout &rarr;</a>
...
<?else:?>
	<li><a href="<?=join_paths(Kohana::config('admin.base'), $path)?>/"<?=strpos(url::current(), $path) !== FALSE ? ' class="selected"' : ''?>><?=$name?></a></li>
<?endif;endforeach;?>
		</ul>
	</nav>
</header>
